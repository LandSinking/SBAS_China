...
def create_parser():
    ...
    parser.add_argument('--dtype',
                        '--data-type',
                        dest='data_type',
                        choices=DATA_TYPE_STR2OBJ.keys(),
    ...
    parser.add_argument('--force',
                        dest='force',
                        action='store_true',
                        help='enforce output data overwrite.')
    return parser
...
def cmd_line_parse(iargs=None):
    ...
    if os.path.isfile(inps.outfile) and not inps.force:
        raise FileExistsError(
            'Output file already exists! To overwrite, re-run with "--force" option.')
...
def main(iargs=None):
    ...
    if num_file == 1:
        ...
        dsDict[inps.dset_name] = np.zeros((num_file, box[3] - box[1], box[2] - box[0]),
                                          dtype=inps.data_type)
